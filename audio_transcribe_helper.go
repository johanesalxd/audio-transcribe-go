...
func transcribe(ctx context.Context, client *speech.Client, bqReq *BigQueryRequest) *BigQueryResponse {
	var transcripts []string

	for _, call := range bqReq.Calls {
		audio := &speechpb.RecognitionAudio{
			AudioSource: &speechpb.RecognitionAudio_Uri{
				Uri: fmt.Sprint(call[0]),
			},
		}
		...
		if err != nil {
			...
			transcripts = append(transcripts, transcript.toJSONString())
